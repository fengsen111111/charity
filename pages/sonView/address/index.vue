<template>
	<view class="">
		<hearch :title="'收货地址'" :isLeft="true" />
		<view class="p-3 text12">
			<view class="bg-whilt mt-3 rending1 p-3" v-for="item in dataList" :key="item.id">
				<view class="flex">
					<text>{{item.name}}</text>
					<text class="space-x-2">{{item.mobile}}</text>
				</view>
				<view class="flex justify-between">
					<view class="w80">
						{{item.address}}
					</view>
					<view class="text-center mr-2">
						<uni-icons type="location" size="20" color="#4867CF"></uni-icons>
						<view class="col999">
							{{item.distance}}
						</view>
					</view>
				</view>
				<view class="flex justify-between items-center">
					<view class="flex items-center">
						<uni-icons :type="item.id==1?'checkbox-filled':'circle'" size="20" color="#4867CF"></uni-icons>
						<view class="space-x-1">默认收货地址</view>
					</view>
					<view class="flex">
						<uni-icons type="compose" size="20" color="#4867CF"></uni-icons>
						<uni-icons type="trash" size="20" class="space-x-3" color="#FC6265"
							@click="toggle('center',item)"></uni-icons>
					</view>
				</view>
			</view>
			<view class="h10"></view>
			<view class="h-2"></view>
			<view class="bg486 text-whlie w-full py-3 rending2 text-center text16"
				@click="handleUrl('/pages/sonView/addressForm/index')">
				新增地址
			</view>
		</view>
		<!-- 删除弹框 -->
		<uni-popup ref="popup" background-color="#fff" borderRadius="0.5rem 0.5rem 0.5rem 0.5rem">
			<view class="bgF9 p-4 overflowAuto rending2" style="width: 17rem;">
				<view class="flex justify-between">
					<view class="w-4"></view>
					<view class="text16">
						是否删除地址
					</view>
					<view class="" @click="close">
						<uni-icons type="closeempty" size="20"></uni-icons>
					</view>
				</view>
				<view class="text12">
					<view class="bg-whilt mt-3 rending1 p-3">
						<view class="flex">
							<text>张三</text>
							<text class="space-x-2">13512341234</text>
						</view>
						<view class="flex justify-between">
							<view class="w-full">
								四川省成都市武侯区世纪城天府大道中段588号天天小区快鸟驿站
							</view>
						</view>
					</view>
				</view>
				<view class="flex justify-between mt-3">
					<view class="bg486 text-whlie text-center py-1 rending1 twoBtn" @click="close">
						再想想
					</view>
					<view class="bgFC6 text-whlie text-center py-1 rending1 twoBtn" @click="_deleteAddress">
						删除
					</view>
				</view>
				<!-- 留白 -->
			</view>
		</uni-popup>
	</view>
</template>

<script>
	import hearch from "@/components/hearch/index.vue"
	import {
		getAddressList,//列表
		deleteAddress,//删除
		} from '@/request/api.js'
	export default {
		components: {
			hearch
		},
		data() {
			return {
				active: false,
				value: '',
				indexId: '',//删除项id
				dataList: [{
					"id": "1",
					"name": "张三",
					"mobile": "13122112121",
					"address": "四川省成都市武侯区世纪城天府大道中段588号天天小区快鸟驿站",
					"location": "1111,11111",
					"distance": "1.1km"
				},
				{
					"id": "2",
					"name": "李四",
					"mobile": "13122112121",
					"address": "四川省成都市武侯区世纪城天府大道中段588号天天小区快鸟驿站",
					"location": "1111,11111",
					"distance": "1.1km"
				},
				{
					"id": "3",
					"name": "李四",
					"mobile": "13122112121",
					"address": "成华大道二仙桥",
					"location": "1111,11111",
					"distance": "1.1km"
				},
				]
			};
		},
		onLoad() {},
		onShow() {

		},
		onHide() {},
		methods: {
			// 地址列表
			_getAddressList(){
				getAddressList({
					post_params:{
						currentPage:'',//当前页码  
						perPage:'',//分页数量
						store_id:'',//当前门店ID  
					}
				}).then((res)=>{
					console.log('地址列表',res);
				})
			},
			// 删除
			_deleteAddress(){
				_deleteAddress({
					post_params:{
						address_id:''
					}
				}).then((res)=>{
					console.log('删除成功',res);
				})
			},
			handleUrl(url) {
				console.log(url);
				uni.navigateTo({
					url: url + ''
				})
			},
			// 关闭
			close() {
				this.$refs.popup.close()
			},
			// 弹框
			toggle(type,item) {
				this.type = type
				// open 方法传入参数 等同在 uni-popup 组件上绑定 type属性
				this.$refs.popup.open(type)
				this.indexId = item.id //删除id
			},
		}
	};
</script>

<style>
	.twoBtn {
		width: 48%;
	}
</style>